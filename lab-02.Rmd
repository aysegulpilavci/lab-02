...
```{r}
mean_val_bellevue <- mean(bellevue_homes$total_val, na.rm = TRUE)
var_val_bellevue <- var(bellevue_homes$total_val, na.rm = TRUE)
...
```{r}
bellevue_lake_homes <- bellevue_homes %>%
  filter(view_lakewash == 1 & view_lakesamm == 1)

mean_val_bellevue_lake <- mean(bellevue_lake_homes$total_val, na.rm = TRUE)
var_val_bellevue_lake <- var(bellevue_lake_homes$total_val, na.rm = TRUE)

mean_val_bellevue_lake
var_val_bellevue_lake
```

Report:\

In Bellevue, the average home value is around \$1.77 million, while homes with lake views average about \$2.42 million. Lake views make homes more expensive, and their prices also vary more.
...
seattle_summary <- kingco_homes_clean %>%
  filter(city == "SEATTLE", !is.na(sqft), sqft > 0) %>%
  mutate(total_per_sqft = (land_val + imp_val) / sqft) %>%
  group_by(condition) %>%
  summarise(avg_total_per_sqft = mean(total_per_sqft, na.rm = TRUE),
            count = n())
...
```{r}
load("kingco_sales.rda")
```

```{r}
bellevue_after2020 <- kingco_sales %>%
  filter(city == "BELLEVUE", sale_date >= as.Date("2020-01-01"))
```

```{r}
View(bellevue_after2020)
```
...
bellevue_homes <- kingco_homes %>%
  ...
  mutate(total_val=land_val+imp_val) %>%
  arrange(desc(total_val)) %>%
  summarise(avg_total_val=mean(total_val,na.rm=TRUE))
